...
def check_condition_9(point_array, C_PTS, D_PTS, epsilon):
    ...
    for i in range(len(point_array) - C_PTS - 1 - D_PTS - 1):
        if point_array[i] == point_array[i + C_PTS + 1] or point_array[i + C_PTS + 1] == point_array[i + C_PTS + 1 + D_PTS + 1]:
            continue
        angle = get_angle(point_array[i], point_array[i + C_PTS + 1], point_array[i + C_PTS + 1 + D_PTS + 1])
